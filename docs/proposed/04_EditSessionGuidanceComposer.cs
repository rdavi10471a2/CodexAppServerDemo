// =============================================================================
// EditSessionGuidanceComposer.cs
// Part of: CodeHands_Implementation_Proposal.md
// Location: docs/proposed/
// Description: Generates tool priority guidance for prompts and session bootstrap
...
/// <summary>
/// Composes guidance text for prompts and session bootstrap.
/// </summary>
public sealed class EditSessionGuidanceComposer
{
    /// <summary>
    /// Composes the edit session guidance including tool priority by file type.
    /// </summary>
    ...
    /// <summary>
    /// Composes the session bootstrap capability manifest.
    /// </summary>
    public string ComposeSessionBootstrapCapabilityManifest()
    {
        ...
        builder.AppendLine("### Phase Guidance");
        builder.AppendLine("- **DISCOVERY**: Analyze workspace, propose files");
        builder.AppendLine("- **EDITING**: Make edits via MCP tools");
        builder.AppendLine("- **OVERLAY BUILD**: Wait for build verification");
        builder.AppendLine("- **MERGE PENDING**: Human reviews and merges files");
        builder.AppendLine("- **DISCUSSION NEEDED**: File(s) rejected, discuss with user");
        builder.AppendLine("- **MERGE COMPLETE**: All accepted, ready for rebuild");
        
        return builder.ToString();
    }
    
    /// <summary>
    /// Composes phase-specific guidance.
    /// </summary>
    public string ComposePhaseGuidance(WorkflowPhase phase, EditSession? editSession)
    {
        return phase switch
        {
            ...
            WorkflowPhase.Editing => 
                $"Phase: EDITING - Make edits using MCP tools. {editSession?.Files.Count ?? 0} files in session.",
            ...
            WorkflowPhase.MergePending => 
                $"Phase: MERGE PENDING - Awaiting human review.",
            WorkflowPhase.DiscussionPending => 
                "Phase: DISCUSSION NEEDED - File(s) rejected. Discuss with user.",
